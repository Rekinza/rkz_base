...
class MW_RewardPoints_Block_Adminhtml_Catalogrules_Edit_Tab_Form extends Mage_Adminhtml_Block_Widget_Form
{
  protected function _prepareForm()
  {
      $form_program_detail = new Varien_Data_Form();
      $this->setForm($form_program_detail);
      $fieldset = $form_program_detail->addFieldset('rewardpoints_form', array('legend'=>Mage::helper('rewardpoints')->__('Rule Information')));

      $fieldset->addField('rule_auto_apply', 'hidden', array(
            'name' => 'rule_auto_apply',
        ));
      ...
      if ( Mage::getSingleton('adminhtml/session')->getDataCatalogRules() )
      {
          $form_program_detail->setValues(Mage::getSingleton('adminhtml/session')->getDataCatalogRules());
          Mage::getSingleton('adminhtml/session')->setDataCatalogRules(null);
      } elseif ( Mage::registry('data_catalog_rules') ) {
        $form_program_detail->setValues(Mage::registry('data_catalog_rules')->getData());
