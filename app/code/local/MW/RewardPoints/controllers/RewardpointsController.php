...
class MW_RewardPoints_RewardpointsController extends Mage_Core_Controller_Front_Action
{
	...
	{	
		...
		$this->getLayout()->getBlock('head')->setTitle($this->__('KinzaCash'));
	...
	public function fetchcashAction(){
		if (!Mage::getSingleton('customer/session')->isLoggedIn()){
		Mage::getSingleton('core/session')->addError("An error occurred.Please try again");
		}
		else{
		$customer=Mage::getSingleton("customer/session")->getCustomer();
		$params=$this->getRequest()->getParams();
		$amount=$params['amount'];
		$type=$params['cashout_type'];
		$name=NULL;
		$account=NULL;
		$bankName=NULL;
		$branchName=NULL;
		...
		/*$message="Name:".$name.";"."Account:".$account.";"."Bank Name:".
		$bankName.";Branch Name".$branchName.";IFSC:".$ifsc.";Account Type:"
		.$account_type;*/
		}
		else if($type=="charity"){
		//$message="name:".$params['name_cha'];
		$name = $params['name_cha'];
		}
		/*else if ($type=="cheque"){       //This option not available to user
			$address=$params['address1'];
		$address2=$params['address2'];
		$mobile=$params['mobileno'];
		$pincode=$params['pincode'];
		$city=$params['city'];
		$message="Name:".$params['name_ch'].";Cheque No:".$params['cheque']
				 .";IFSC".$params['ifsc_ch'].";Address:".$address."\n".$address1.";Mobile:".$mobile.";Pincode:".$pincode
				 .";Mobile No:".$mobile.";City:".$city;
		}
		else if ($type=="sms"){            //This option not available to user
		$message="Name:".$params['name_sms'].";Mobile No:".$params['mobileno_sms'];
		}*/
		...
		Mage::getSingleton('core/session')->addError("An error occurred.Please try again");
			...
			if ($type){
				//Mage::getModel("rewardpoints/customer")->fetchCash($type,$message,$amount);
				Mage::getModel("rewardpoints/customer")->fetchCash($type,$name,$account,$bankName,$branchName,$ifsc,$account_type,$amount);
