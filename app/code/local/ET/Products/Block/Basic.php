...
class ET_Products_Block_Basic extends Mage_Catalog_Block_Product_Abstract {
    ...
    protected $_template = 'et/products/default.phtml';
    ...
    public function __construct ($attributes=array()) {
        ...
        $this->addData(array(
            ...
            'cache_tags'        => array('products_basic'),
        ));
    }
    ...
    public function getConfig ($name=null, $default=null) {
        return is_null($name)
            ...
            : (
                $this->hasData($name)
                ? $this->getData($name)
                : $this->getDataSetDefault($name, $default)
            );
    }
    ...
    protected function _initConfig($attributes=array()){
        if (!$this->getData('config')){
            if ($store_config = Mage::getStoreConfig('et_products_configs')) {
    ...
    protected function _getItems ($data_source = null) {
        /* @var $helper ET_Products_Helper_Data */
