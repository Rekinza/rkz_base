...
class ET_Oxynic_Helper_Data extends Mage_Core_Helper_Abstract {
	
	public $_enable_cpanel = true;
	public $_theme_config = null;
	
	protected function _getConfig(){
		if ( !isset($this->_theme_config) ){
			$this->_theme_config = new Varien_Object();
			if ( $store_config = Mage::getStoreConfig('et_oxynic_configs') ){
				foreach ( $store_config as $group => $values ){
					foreach ( $values as $k => $v ){
						$this->_theme_config->setData($k, $v);
					}
				}
			}
			$this->_enable_cpanel = $this->_theme_config->getDataSetDefault('show_cpanel', true);
		}
		return $this->_theme_config ? $this->_theme_config : new Varien_Object();
	}
	
	public function getAllConfig (){
		return $this->_getConfig()->getData();
	}
	
	public function getConfig ($name, $default=null) {
		if ( $this->_enable_cpanel ){
			$_cookie_key = $this->getCookieName( $name );
			if ( isset($_COOKIE[$_cookie_key]) ){
				return $_COOKIE[$_cookie_key];
			}
		}
	...
	public function getCookieName ($name) {
		...
		return 'oxynic_' . $name . '_in_' . $store_id;
	}
	...
	public function saveConfig ($name, $value) {
		if ($this->_enable_cpanel) {
			...
			$cookie->set($_cookie_key, $value ,time()+15*60);
			return true;
		}
	...
	public function getHomeBannerId(){
		return $this->getConfig('home_banner_id');
	}
	
	public function getCssSuffix($color='', $responsive=true){
	    if (!$color){
	        $color = $this->getConfig('theme_color', 'default');
	    }
	    if (is_null($responsive)){
	        $responsive = $this->getConfig('theme_responsive', true);
	    }
	    $sf = $color ? '-'.$color : '';
	    $sf .= $responsive ? '-nr' : '';
	    return $sf;
	...
	public function getAppStylesheet(){
		return 'css/app-'.$this->getConfig('theme_color', 'default').'.css';
	}
	
	public function getPositionId(){
		return $this->getConfig('position_id');
	}
	
	public function getGoogleFonts(){
	    if (!$this->getConfig('use_google_font', true)) {
	        return false;
	    }
	    $google_font = $this->getConfig('google_font');
	    $google_font = trim($google_font);
	    if (!empty($google_font)){
	        return urlencode($google_font);
	    }
	    return false;
	}
	...
	public function getStoreCss(){
	    ...
	    $font_family = $this->getConfig('font_family');
	    $font_size = $this->getConfig('font_size');
	    $text_color = $this->getConfig('text_color');
	    $bg_color = $this->getConfig('background_color');
	    $bg_style = $this->getConfig('background_style');
	    if ($bg_style=='pattern') {
	        $bg_style_pattern = $this->getConfig('background_pattern');
	        $bg_style_pattern_url = $bg_style_pattern ? Mage::getBaseUrl('media') . 'et/pattern/' . $bg_style_pattern . '.png' : '';
	        $bg_style = $bg_style_pattern ? 'background-image: url(' . $bg_style_pattern_url . ');' : '';
	    } else if($bg_style=='image') {
	        $bg_style_image = $this->getConfig('background_image', '');
	        $bg_style_repeat = $this->getConfig('background_repeat', '');
	        $bg_style_position = $this->getConfig('background_position', '');
	        $bg_style = $bg_style_image ? sprintf('background-image: url(%s) %s %s ;', $bg_style_image, $bg_style_repeat, $bg_style_position) : '';
	    }
	    
	    ob_start();
	    $body_selector = "body";
	    if ($layout = $this->getConfig('theme_layout')){
	        $body_selector .= '.layout-'.$layout;
	    }
	    if ($font_family || $font_size || $text_color) {
	        echo "
	        body {
	    ...
	    if ($link_color = $this->getConfig('link_color')) {
	        echo "
	        $body_selector a{
	           color: #$link_color;
	        }";
	    }
	    if ($link_hover_color = $this->getConfig('link_hover_color')) {
	        echo "
	        $body_selector a:hover, $body_selector a:focus{
	            color: #$link_hover_color;
	        }";
	    }
	    // use google font, and set apply_googlefont_to
	    $apply_googlefont_to = '';
	    if ($this->getConfig('use_google_font', true)){
	        echo "
	        ".$this->getConfig('apply_googlefont_to', '');
	    }
        
        if (!$this->getConfig('show_add2cart')){
            echo "$body_selector .btn-cart, $body_selector .item-addcart { display: none !important; }";
        }
        if (!$this->getConfig('show_add2wishlist')){
            echo "$body_selector .link-wishlist, $body_selector .item-wishlist { display: none !important; }";
        }
        if (!$this->getConfig('show_add2compare')){
            echo "$body_selector .link-compare, $body_selector .item-compare { display: none !important; }";
        }
        if (!$this->getConfig('show_quickview')){
            echo "$body_selector .item-quickview { display: none !important; }";
        }
