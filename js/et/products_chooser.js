var ProductsChooser = new Class.create();
ProductsChooser.prototype = {
    ...
    chooserGridCheckboxCheck: function (b, a, c) {  
        ...
        $(this.element).value = this.chooserSelectedItems.keys().join(", ");
...
// var ProductsChooserManager = new Class.create();
var getProductChooser = function (element, url) {
    if ( element in ProductsChooser ){
        // already loaded
        var chooser_container = $(element + "_chooser");
        chooser_container.toggle();
        return false;
    } else {
        ProductsChooser[element] = new ProductsChooser(element);
    }
    ...
    new Ajax.Request(
        url, {
            ...
            onSuccess: function (b) {
                var chooser_container = $(element + "_chooser");
                chooser_container.update(b.responseText);
                chooser_container.scrollTo();
            }
    });
