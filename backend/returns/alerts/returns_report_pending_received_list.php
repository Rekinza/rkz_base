...
{
	$query =  "SELECT id, email, pickup_date, waybill_number FROM thredshare_returns WHERE pickup_date < '$startdate' AND status = 'picked up' ORDER BY pickup_date";
	...
	if( $numresult > 0 )
	{
		...
		{	
			...
			$pending_message = "<tr><td>id: ".$order_id."</td><td> email : ".$email."</td><td> waybill number: ".$waybill_number."</td><td> No. of Days since picked up: ".$date_diff."</td></tr>\n";
		...
		if (!empty($pending_array))
		{
			...
			$subject = "CRON JOB - Delay in receiving returns";
