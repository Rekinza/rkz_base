...
$query = "SELECT * from thredshare_returns WHERE pickup_date BETWEEN '{$startdate}' AND '{$enddate}' AND waybill_number != ''";
echo $query;

$result = mysql_query($query);
$waybill_status_update_or_email_send_error_list = array();
$cancelled_pickup_email_send_list = array();
...
while($row = mysql_fetch_assoc($result)){
	...
	echo $logistics_partner." ".$email." ".$order_id;
	...
	if($row['status'] == "requested" && ($status == "scheduled"))	
	 	{
			$query1 = "UPDATE thredshare_returns SET status = 'scheduled' WHERE waybill_number = '$awb' ";
			echo $query1."<br>";
	...
	elseif(($row['status']  == 'requested' || $row['status'] == 'cancelled' || $row['status'] == 'scheduled') && ($status == "picked"))
	 	{
			...
			$body = $html;
			$mobile = $row['mobile'];
			$sms_content = "Hi {$customer_name}, We have picked-up your returns package. It can take 7-10 days for us to receive them. Once we receive and check the item(s), we will credit your account with the relevant Kinza Cash. Your can track your package here {$logistic_tracking_link} Cheers, Team Rekinza";

			$flag1 = sendSMS($sms_content, $mobile);
			$flag2 = sendmail($subject , $body, $email);		//pickupmail
			if($flag1 || $flag2)				//if mail is successful
			{
				$query1 = "UPDATE thredshare_returns SET status = 'picked up' WHERE waybill_number = '$awb' ";
				echo $query1."<br>";
				$result1 = mysql_query($query1);
				if ($result1 == 'TRUE')
				{
 	...
 	elseif($row['status'] != "cancelled" && ($status == "cancelled" ))
 	{
		...
		$query1 = "UPDATE thredshare_returns SET status = 'cancelled' WHERE waybill_number = '$awb' ";
		echo $query1."<br>";
		$result1 = mysql_query($query1);
		if ($result1 == 'TRUE')
		{	
			$cancel_pickup = "id: ".$row['id']." table-status: ".$row['status']." ".$logistics_partner." status: ".$status." waybill-number: ".$awb." email: ".$row['email']." return pickup cancelled.\n";
			array_push($cancelled_pickup_email_send_list, $cancel_pickup);
		}
		else
		{
			$fault = "id: ".$row['id']." table-status: ".$row['status']." ".$logistics_partner." status: ".$status." waybill-number: ".$awb." query failed.\n";
...
if (!empty($cancelled_pickup_email_send_list))
{
     
	$email = "vidisha@rekinza.com";
	$cc_email = "stuti@rekinza.com";
	$subject = "CRON JOB -- cancelled return pickups";
	$fault_string = implode("<br>",$cancelled_pickup_email_send_list);
	$body = $fault_string;
    sendmail($subject , $body, $email, $cc_email);
}
