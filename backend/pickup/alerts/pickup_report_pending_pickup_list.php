...
{
	$query =  "SELECT id, email, pick_up_date, waybill_number FROM thredshare_pickup WHERE pick_up_date BETWEEN '$startdate' AND '$yesterday' AND status = 'scheduled' ORDER BY pick_up_date";
	...
	if( $numresult > 0 )
	{
		...
		{	
			...
			$pending_message = "<tr><td>id: ".$pickup_id."</td><td> email : ".$email."</td><td> waybill number: ".$waybill_number."</td><td> Delay in Days: ".$date_diff."</td></tr>";
		...
		if (!empty($pending_array))
		{
			...
			$subject = "CRON JOB - Delayed pickups";
