...
if (1)//$numresult > 0)
{
	...
	if ($qc_status == 'accepted' && $inventory_data_status == 'Not complete')
	{
		...
		$query = "SELECT * FROM `inventory` WHERE qc_status = 'accepted'  AND ( size = '' OR retail_value ='' OR suggested_price ='' OR upload_status ='') " ;
	...
	else if ($qc_status == 'accepted' && $inventory_data_status == 'Complete')
	{
		...
		$query = "SELECT * FROM `inventory` WHERE qc_status = 'accepted' AND ( size != '' AND retail_value != '' AND suggested_price != '' AND upload_status != '' ) " ;
